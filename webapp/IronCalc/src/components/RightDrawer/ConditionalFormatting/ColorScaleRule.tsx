...
const TYPE_OPTIONS = ["number", "percent", "formula", "percentile"] as const;

const MIDPOINT_TYPE_OPTIONS = [...TYPE_OPTIONS, "none"] as const;
const MIN_TYPE_OPTIONS = ["min", ...TYPE_OPTIONS] as const;
const MAX_TYPE_OPTIONS = ["max", ...TYPE_OPTIONS] as const;

const STOP_TYPE_OPTIONS = [...TYPE_OPTIONS, "none", "min", "max"] as const;

type StopType = (typeof STOP_TYPE_OPTIONS)[number];
...
function getOptionsFromType(type: StopRowProps["type"]): readonly StopType[] {
  switch (type) {
    case "min":
      return MIN_TYPE_OPTIONS;
    case "midpoint":
      return MIDPOINT_TYPE_OPTIONS;
    case "max":
      return MAX_TYPE_OPTIONS;
    default:
      return TYPE_OPTIONS;
  }
}
...
const StopRow = ({
  ...
  const typeOptions = options.map((v) => ({
    value: v,
    label: t(`conditional_formatting.color_scale_type_${v}`),
  }));

  const isNone = stop.type === "none";
    ...
    <div className="ic-edit-rule-field-wrapper">
      ...
      <div className="ic-cs-settings-row">
        <Select
          ...
          hideCheck
        />
        {!isNone && stop.type !== "min" && stop.type !== "max" && (
          <Input
            type="text"
            value={stop.value}

...
fn test_font_with_no_color_element_is_none() {
    ...
    let no_color_font = fonts
        .iter()
        .find(|f| f.sz == 8)
        .expect("expected a font with sz=8 (the one with no <color> element)");
    assert_eq!(
        no_color_font.color, None,
        "font with no <color> element should import as None, not Some(\"#000000\")"
    );

    let themed_fonts: Vec<_> = fonts.iter().filter(|f| f.sz == 11).collect();
    assert!(!themed_fonts.is_empty(), "expected at least one sz=11 font");
    for font in themed_fonts {
        assert!(
            font.color.is_some(),
            "font with <color theme=...> should resolve to Some(rgb), got None"
        );
    }
    ...
    let reloaded_no_color = reloaded
        ...
        .iter()
        .find(|f| f.sz == 8)
        .expect("expected sz=8 font after round-trip");

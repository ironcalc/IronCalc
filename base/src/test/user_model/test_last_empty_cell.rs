...
fn basic_tests() {
    ...
    assert_eq!(model.get_last_non_empty_in_row_before_column(0, 3, 4).unwrap(), None);
    // Row 3 before column 5 should be 5
    assert_eq!(model.get_last_non_empty_in_row_before_column(0, 3, 7).unwrap(), Some(5));
}
...
fn test_last_empty_cell() {
    ...
    assert_eq!(model.get_last_non_empty_in_row_before_column(0, 3, 14).unwrap(), None);

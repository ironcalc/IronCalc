...
fn assert_in_step(model: &Model, sheet: usize) {
    ...
    for (index, formula) in shared_formulas.iter().enumerate() {
        let found = lookup.get(formula).copied();
    ...
    assert!(lookup.len() <= shared_formulas.len());
}
...
fn many_different_formulas() {
    ...
    assert_eq!(model.shared_formula_lookup[0].len(), n as usize);
...
fn a_lookup_out_of_step_is_not_believed() {
    ...
    model.shared_formula_lookup[0].insert(formula.clone(), 1);
    model._set("B2", "=A2+1");
    // it is missing a formula that is in the list
    model.shared_formula_lookup[0].remove(&formula);
    model._set("B3", "=A3+1");

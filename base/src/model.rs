...
impl<'a> Model<'a> {
    ...
    pub fn get_fmt_settings(&self) -> FmtSettings {
        ...
        // Number format ("#,##0.###")
        // The CLDR formats are a bit different than Excels
        // self.locale.numbers.decimal_formats.standard.clone();
        let number_fmt = {
            if self.locale.numbers.symbols.group == "," {
                "0.00".to_string()
            } else {
                "0,00".to_string()
            }
        };
        let number_example = format_number(1000.0, &number_fmt, self.locale).text;
        FmtSettings {
            currency,

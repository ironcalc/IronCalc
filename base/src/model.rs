    ...
    pub(crate) fn evaluate_node_in_context(
        ...
        match node {
            OpSumKind { kind, left, right } => match kind {
                OpSum::Add => self.handle_arithmetic(left, right, cell, &|f1, f2| f1 + f2),
                OpSum::Minus => self.handle_arithmetic(left, right, cell, &|f1, f2| f1 - f2),
            },
            ...
            OpProductKind { kind, left, right } => match kind {
                OpProduct::Times => self.handle_arithmetic(left, right, cell, &|f1, f2| f1 * f2),
                OpProduct::Divide => self.handle_arithmetic(left, right, cell, &|f1, f2| f1 / f2),
            },
            OpPowerKind { left, right } => {
                self.handle_arithmetic(left, right, cell, &|f1, f2| f1.powf(f2))
            }

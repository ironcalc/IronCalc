...
struct CaGCalendarsFormat {
    ...
    stand_alone: HashMap<String, HashMap<String, String>>,
}
#[derive(Serialize, Deserialize, Encode)]
...
pub fn get_dates_formatting(cldr_dir: &str, locale_id: &str) -> Result<Dates, &'static str> {
    ...
    // See: https://cldr.unicode.org/translation/date-time/date-time-patterns
    // for the difference between stand-alone and format. We will use only the stand-alone mode
    let months_stand_alone = &gregorian.months.stand_alone;
    let days_stand_alone = &gregorian.days.stand_alone;
    ...
    for index in month_index {
        months_letter.push(months_stand_alone["narrow"][index].to_owned());
        months_short.push(months_stand_alone["abbreviated"][index].to_owned());
        months.push(months_stand_alone["wide"][index].to_owned());
    }

    let day_index = vec!["sun", "mon", "tue", "wed", "thu", "fri", "sat"];
    for day in day_index {
        day_names_short.push(days_stand_alone["abbreviated"][day].to_owned());
        day_names.push(days_stand_alone["wide"][day].to_owned());
    }
